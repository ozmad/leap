...
**Button row layout — Cancel bottom-left, primary bottom-right.** Project convention for every `QDialog` with a Cancel button: add Cancel first, then `addStretch()`, then the primary action(s) on the right:
...
Do **not** use `QDialogButtonBox(Ok | Cancel)` for new dialogs — on macOS it groups Cancel next to OK on the right, which violates the convention. For 3-button cases (e.g. Cancel + secondary + primary), keep Cancel on the outside-left and group the other two on the right of the stretch — see `_mixins/actions_menu_mixin.py` and `dialogs/git_changes_dialog.py:CommitListDialog`. Close-labeled dismissal buttons (one-button viewer dialogs like `WhatsNewDialog`, `NotesDialog`) are not covered by this rule.
